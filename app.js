...
const Planet = ({id, url, name, desc, diameter, moons, hidden=false, hide=()=>{}}) => (
  <div className="card" style={{display: (hidden? 'none':'')}}>
    <div><img src={url} alt={name}></img></div>
    <h2>{name}</h2>
    <p>{desc}</p>
...
const Container = ()=>{
  ...
  <div className='container'>
    ...
    <Planet 
      key={p.id} {...p} 
      hide={(id)=>setFilteredPlanets(filteredPlanets.filter(p=>p.id!=id))}/>))}
  </div>
)}
